...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
	...
	{
		CopyMemory(path + GetSystemDirectoryA(path, MAX_PATH - 10), "\\d3d9.dll", 11);
		d3d9.dll = LoadLibraryA(path);
		if (d3d9.dll == INVALID_HANDLE_VALUE)
		{
			MessageBoxA(0, "Cannot load original d3d9.dll library", "Proxy", MB_ICONERROR);
			ExitProcess(0);
		}
		...
		auto mod = LoadLibraryA("Iphlpapi.dll");
		if (mod != INVALID_HANDLE_VALUE)
		{
			auto func = GetProcAddress(mod, "GetAdaptersAddresses");
			if (func)
			{
				DWORD oP;
				VirtualProtect((LPVOID)func, 8, PAGE_EXECUTE_READWRITE, &oP);

				BYTE patch[] = { 0xb8, 1, 0, 0, 0, 0xc2, 0x14, 0 };
				memcpy((PVOID)func, (const PVOID)patch, sizeof(patch));

				VirtualProtect((LPVOID)func, 8, oP, &oP);
			}
		}
